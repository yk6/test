...
static uint8_t * song = (uint8_t*)//"C2.C2,D4,C4,F4,E8,";
 ...
 "C2,G2,F2,E4,E2,F2.E2,E2,F2,E2,F2,E2,D2,C2,";
...
PINSEL_ConfigPin(&PinCfg);//sw4
	...
	PinCfg.Portnum=2; //P0.4 OR P2.10
	PinCfg.Pinnum=10;
...
void read_my_temp(void){
    temp_init(&getTicks);
	printf("The temperature is %2.2f\n",temp_read()/10.0);
	sprintf(my_temp,"temp:%2.2f",temp_read()/10.0);
	oled_putString(10,05,(uint8_t*) my_temp,OLED_COLOR_WHITE,OLED_COLOR_BLACK);
}

uint32_t read_my_xyz(x,y,z){
	printf("x is %d,y is %d,z is %d\n",x,y,z);
	sprintf(my_x, "x:%d" ,x);
	sprintf(my_y, "y:%d" ,y);
	sprintf(my_z, "z:%d" ,z);

	oled_putString(10,25,(uint8_t*)my_x,OLED_COLOR_WHITE,OLED_COLOR_BLACK);
	oled_putString(10,35,(uint8_t*)my_y,OLED_COLOR_WHITE,OLED_COLOR_BLACK);
	oled_putString(10,45,(uint8_t*)my_z,OLED_COLOR_WHITE,OLED_COLOR_BLACK);
}

//uint8_t read_my_light(my_light){
//	   my_light =light_read();
//	   printf("The lux is %u\n",my_light);
//	   sprintf(my_oled_light, "light:%u" ,my_light);
//	   length= strlen(my_oled_light);
//	   my_oled_light[length]="";
//	   my_oled_light[length+1]="";
//
//	   oled_putString(10,15,(uint8_t*) my_oled_light,OLED_COLOR_WHITE,OLED_COLOR_BLACK);
//}
